...
class ReservationServiceImpl implements ReservationService {
  ...
  public void reservationToLecture(ReservationRequest request) {
    Optional<LectureEntity> lecture = lectureRepository.findLectureEntityBySubject(request.title_lecture());
    lecture.ifPresent(e -> {
      int numberOfOccupiedPlaces = reservationRepository.countAllByLectureId(e.getId());
    ...
    user.ifPresentOrElse(
        e -> makeReservationWhenUserExists(e, lectureEntity)
  ...
  private void makeReservationWhenUserDoesNotExist(UsersEntity usersEntity, LectureEntity lecture) {
    int numberOfUsers = usersRepository.countUsersEntitiesByLogin(usersEntity.getLogin());
    if (numberOfUsers != 0) {
  ...
  private void makeReservationWhenUserExists(UsersEntity usersEntity, LectureEntity lecture) {
    List<UUID> lecturesId = lectureRepository.findLectureEntitiesByTimes(lecture.getTimes()).stream().map(LectureEntity::getId).toList();
    List<ReservationEntity> reservations = reservationRepository.findAllByUserId(usersEntity.getId());
    reservations.forEach(e -> {
      if (lecturesId.contains(e.getLectureId()))
  ...
  private void makeReservationLecture(LectureEntity lecture, UsersEntity usersEntity) {
